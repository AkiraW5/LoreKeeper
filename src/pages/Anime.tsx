...
import { Anime, MEDIA_GENRES, RATING_LABELS } from '../types';
import { generateId, getRatingColor, getStatusColor } from '../lib/utils';

const defaultItem: Omit<Anime, 'id' | 'created_at' | 'updated_at'> = {
  title: '', genre: 'Ação', type: 'TV',
  seasons_watched: 0, total_seasons: 1,
  episodes_watched: 0, total_episodes: 0,
  status: 'Planejado', rating: null, studio: '',
  start_date: '', end_date: '', notes: '',
  cover_url: '', overview: '',
};

const STATUS_LIST = ['Assistindo', 'Concluído', 'Dropado', 'Pausado', 'Planejado'] as const;
const ANIME_TYPES = ['TV', 'Filme', 'OVA', 'ONA', 'Especial'] as const;
...
export default function AnimePage() {
  ...
  useEffect(() => { loadItems(); }, []);
  async function loadItems() { setItems(await window.api.db.getAll('animes', 'updated_at DESC')); }

  function openAdd() { setEditing(null); setFormData({ ...defaultItem }); setModalOpen(true); }
  function openEdit(item: Anime) {
    setEditing(item);
    setFormData({
      title: item.title, genre: item.genre, type: item.type,
      seasons_watched: item.seasons_watched, total_seasons: item.total_seasons,
      episodes_watched: item.episodes_watched, total_episodes: item.total_episodes,
      status: item.status, rating: item.rating, studio: item.studio,
  ...
  async function save() {
    ...
    if (editing) await window.api.db.update('animes', editing.id, data);
    else await window.api.db.insert('animes', { id: generateId(), ...data, created_at: new Date().toISOString() });
    setModalOpen(false); loadItems();
  }

  async function deleteItem() {
    if (!deleteTarget) return;
    await window.api.db.delete('animes', deleteTarget.id);
  ...
  return (
    <div className="p-6 flex flex-col h-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-dark-100 flex items-center gap-2">
            <Play size={24} className="text-red-400" /> Animes
          </h1>
          <p className="text-sm text-dark-400 mt-1">{items.length} animes rastreados</p>
        </div>
        <button onClick={openAdd} className="btn-primary flex items-center gap-2"><Plus size={16} /> Adicionar Anime</button>
      </div>

      <div className="flex items-center gap-3 mb-4">
        <div className="w-64"><SearchBar value={search} onChange={setSearch} placeholder="Buscar anime..." /></div>
      ...
      <div className="flex-1 overflow-auto">
          ...
          {filtered.map(item => (
            <article key={item.id} className="group bg-dark-800/50 border border-dark-700/50 rounded-xl overflow-hidden hover:border-accent-500/40 transition-all duration-200">
              ...
              <div className="p-3">
                ...
                <p className="text-xs text-dark-400 mt-1 line-clamp-1">{item.genre} • {item.type} {item.studio && `• ${item.studio}`}</p>
                ...
                <div className="mt-2 space-y-1.5">
                  ...
                  <div className="w-full h-1.5 bg-dark-700 rounded-full overflow-hidden">
                    <div className="h-full bg-red-500 rounded-full" style={{
                      width: `${item.total_episodes > 0 ? (item.episodes_watched / item.total_episodes) * 100 : 0}%`
                    }} />
                  </div>
                </div>
                {item.rating && (
        ...
        {filtered.length === 0 && <div className="text-center text-dark-400 py-12">Nenhum anime encontrado</div>}
      </div>

      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar Anime' : 'Adicionar Anime'}>
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <ApiSearchBox
              value={formData.title}
              onChange={title => setFormData(f => ({ ...f, title }))}
              placeholder="Ex: Naruto, Attack on Titan..."
              label="Título"
          ...
          <div>
            <label className="block text-xs font-medium text-dark-300 mb-1">Estúdio</label>
            <input type="text" value={formData.studio} onChange={e => setFormData(f => ({ ...f, studio: e.target.value }))} className="input-field" />
          </div>
          <div>
      ...
      <ConfirmDialog isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={deleteItem} title="Excluir Anime" message={`Excluir "${deleteTarget?.title}"?`} />
    </div>
  );
}
